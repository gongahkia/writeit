...
- MCP support is limited to stdio and Streamable HTTP tools/resources/prompts plus OAuth PKCE browser handoff, localhost callback capture, refresh-token rotation, and stdio sampling/elicitation when explicit client handlers are injected. The app UI still does not expose sampling review or elicitation forms.
