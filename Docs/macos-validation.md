...
21. For an OAuth-protected Streamable HTTP MCP server, run `mcp.oauth.start`, complete the browser authorization, then run `mcp.oauth.exchange` and confirm later MCP HTTP calls attach the stored bearer token.
...
- MCP support is limited to stdio and Streamable HTTP tools/resources/prompts plus OAuth PKCE browser handoff; automatic redirect capture and refresh-token rotation are not implemented.
