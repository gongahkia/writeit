...
- Approved file-search folder paths render in monospaced caption text with one-line middle truncation.
...
- MCP server health rows cap long server state text to two lines.
...
- Transcript, memory, and audit rows cap long request, response, and payload text to bounded line counts.
