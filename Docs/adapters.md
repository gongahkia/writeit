...
A local Python virtual environment for future toolkit dependency installation was created at `.dist/adapter-training/venv` on 2026-06-19 and verified with Python 3.14.6. The toolkit itself is still absent, so `ADAPTER_TOOLKIT_DIR` remains blocked until the Apple download is available.
