...
- `mcp.call`, `mcp.resources.list`, `mcp.resource.read`, `mcp.prompts.list`, and `mcp.prompt.get` are default-off and support configured MCP stdio or Streamable HTTP servers.
...
- MCP support covers stdio and basic Streamable HTTP tools, resources, and prompts; sampling, elicitation, OAuth, and dynamic native tool schemas are not implemented.
