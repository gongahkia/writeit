...
- `mcp.call`, `mcp.resources.list`, and `mcp.resource.read` are default-off and support configured MCP stdio or Streamable HTTP servers.
...
- MCP support covers stdio and basic Streamable HTTP tools and resources; prompts, sampling, elicitation, OAuth, and dynamic native tool schemas are not implemented.
