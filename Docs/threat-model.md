...
OCR text, screen snapshot metadata, barcode payloads, UI labels, and prior conversation text can contain hostile instructions. The app wraps tool payloads as untrusted blocks and escapes matching closing delimiters before model ingress. New tool families must keep payload data separate from system/developer instructions.
