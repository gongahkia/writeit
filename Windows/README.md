...
It provides local Windows Ink handwriting recognition for the same six Latin-language choices as macOS, a global `Ctrl+Alt+Space` toggle shortcut, mouse and pen ink capture, encrypted local history, and a best-effort Ctrl+V delivery path that always leaves the result on the clipboard.
...
Windows handwriting recognition depends on installed handwriting language packs. If the selected language is absent, WriteIt uses English when it is installed; otherwise capture stays disabled.
